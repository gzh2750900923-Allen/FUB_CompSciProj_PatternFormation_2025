...
# Import the core engine function responsible for advancing the time step from our previously written explicit solver file
...
def simulate_and_visualize():
    ...
    dt = 0.2              # Set the time step size to satisfy the CFL stability condition and prevent calculation blow-up
    steps = 8000          # Set the total number of time steps (frames) to simulate; more steps mean a more mature pattern
    ...
    print("🚀 Simulation starting, please wait...")
    ...
    for i in range(steps):
        ...
        # This perfectly adapts to the generic interface we defined in explicit_solver.py
        current_f = lambda u_mat, v_mat: f_GS(u_mat, v_mat, alpha)
        current_g = lambda u_mat, v_mat: g_GS(u_mat, v_mat, alpha, beta)
        
        # Call our explicit engine to calculate the next frame of u and v
        u_next, v_next = explicit_euler_step(u, v, dx, dt, D_u, D_v, current_f, current_g)
        
        # Update the "future" state calculated to the "current" state, ready to enter the next loop
        u = u_next
        v = v_next
        
        # Print the progress in the terminal every 1000 steps so you know the program hasn't frozen
        if i % 1000 == 0:
    ...
    plt.title("Grey-Scott Turing Pattern (Explicit Method)") # Add a title to the picture
